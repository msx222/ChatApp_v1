...
def show_home_markdown():
    st.markdown(
        """
        ## 🚗 業務サポートAI

        このアプリは、品情二の業務を支援するための **AI統合チャットアプリ** です。

        ---
        ### 🧠 利用できるAIモード
        - **通常チャット**: LLMへの問合わせ
        - **法令適合判定**： 道路運送車両の保安基準への適合条文の抽出  
        - **PDF解析**: PDFの内容を構造化・要約  
        - **関連文書検索**：ポチットっとチャットによる所内文章の検索  
        - **異議申請処理**：異議申請を自動作成

        ---
        ### 💡 まずはモードを選んで質問してみてください
        画面右上の **メニュー（モード選択）** からエージェントを切替えれます。
...
with st.sidebar:
    ...
    mode = st.radio(
        "✨AIモード選択",
        ["通常チャット", "法令適合判定", "PDF解析", "ポチットっとサーチ", "ローカルスクリプト実行"],
    )
    st.markdown("---")
    # mode = st.radio(
    #     "🛠️自動化ツール",
...
def list_local_scripts():
    ...
    full_paths = [os.path.join(scripts_dir, f) for f in files]
...
def run_local_script(script_path, args=""):
    try:
        ...
        cmd = [python_exec, script_path] + args.split()
...
if prompt := st.chat_input("メッセージを入力してください", accept_file="multiple"):
    ...
    if st.button("hello.pyを実行"):
        p = subprocess.Popen(["scripts/hello.bat"], shell=True)


        # 非ブロッキングで stderr を読む
        threading.Thread(target=read_stderr, args=(p,), daemon=True).start()

        st.success("hello.py を起動しました（GUIを確認してください）")
    # -------------------------
    # 通常チャット / PDF解析 → GPT 応答
    # -------------------------
    else:
